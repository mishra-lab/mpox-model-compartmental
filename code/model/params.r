...
.dn = list( # global variable storing dimnames
  city    = c('A','B'),
  risk    = c('high','low'),
  health  = c('sus','vax','exp','inf','iso','rec')
)

def.t = function(t0=0,t1=90,dt=1){
...
def.params.fixed = function(){
  ...
  P$X0.eih        =     10 # number initially exposed/infected
  P$x0.eih.high   =   1.00 # proportion in higher risk among initially exposed/infected/isolating
  P$x0.eih.A      =   0.50 # proportion in city A among initially exposed/infected/isolating
  # mixing + contacts
  ...
  P$C.sex.high.A  =   .189 # daily sexual partnerships among high risk in city A
  P$C.sex.high.B  =   .189 # daily sexual partnerships among high risk in city B
  ...
  P$sar.com       =   0.01 # transmission probability per community encounter
  P$dur.exp       =      8 # incubation period
  P$dur.inf       =     21 # infectious period
...
def.params.cond = function(P){
  # TODO: something funky here with new state
  ...
  P$x0.eih.city      = c('A'=P$x0.eih.A,'B'=1-P$x0.eih.A)
  P$x0.eih.risk      = c('high'=P$x0.eih.high,'low'=1-P$x0.eih.high)
  P$x0.eih.city.risk = array(outer(P$x0.eih.city,P$x0.eih.risk),c(2,2),.dn[c('city','risk')])
  ...
  # print(rowSums(P$M.sex,dim=2)) # DEBUG == 1
  # print(rowSums(P$M.com,dim=2)) # DEBUG == 1
  # print(rowSums(P$C.sex,dim=2) / P$C.sex.city.risk) # DEBUG == 1
  # print(rowSums(P$C.com,dim=2) / P$C.com.city.risk) # DEBUG == 1
...
def.params.toroth = function(){
  P = list()
  P$X             = 100000
  P$x.A           = .75
  P$x0.eih.A      = 1.00
  P$C.sex.high.A  = .245 # R0 = 2.0
  P$C.sex.high.B  = .189 # R0 = 1.5
  P$asso.city = .9
  P$vax.x.A = 'opt'
  return(P)
}
...
def.vax.city.risk = function(P){
  ...
  # print(sum(X.vax.A)+sum(X.vax.B)) # DEBUG == P$X
  # print(sum(X.vax.A[,1])+sum(X.vax.B[,1])) # DEBUG == P$vax.X
  return(array(c(X.vax.A[1,1],X.vax.B[1,1],X.vax.A[2,1],X.vax.B[2,1]),c(2,2),.dn[1:2]))
}

def.X0 = function(P){
  # define initial conditions
  x0.eih      = c(P$dur.exp,P$dur.inf*c(1-P$iso.prop,P$iso.prop))/(P$dur.exp+P$dur.inf)
  X0.eih.city.risk = P$X0.eih * P$x0.eih.city.risk
  X0.data      = outer(P$X.city.risk,c(1,0,0,0,0,0)) # expand into new 'health' dimension
  X0.data[,,1] = X0.data[,,1] - X0.eih.city.risk # remove initial exp/inf/iso from sus
  X0.data[,,3] = X0.eih.city.risk * x0.eih[1] # add back initial exp
  X0.data[,,4] = X0.eih.city.risk * x0.eih[2] # add back initial inf
  X0.data[,,5] = X0.eih.city.risk * x0.eih[3] # add back initial iso
  return(array(X0.data, c(2,2,6), .dn))
}
